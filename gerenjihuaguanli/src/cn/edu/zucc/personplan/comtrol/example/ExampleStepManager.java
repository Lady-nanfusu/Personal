...
public class ExampleStepManager implements IStepManager {
	...
	public void deleteStep(BeanStep step) throws BaseException {
		...
		try {
			...
			String sql="select step_ord,user_id,tbl_plan.plan_id "
					+" form tbl_step,tbl_plan "
			...
			sql = "delete from tbl_step where step id = ?"+step_id;
			st.execute(sql);
			st.close();
			sql = "update tbl_step set step_ord = step_ord -1 where plan_id = ? and step_ord>"+step_ord;
